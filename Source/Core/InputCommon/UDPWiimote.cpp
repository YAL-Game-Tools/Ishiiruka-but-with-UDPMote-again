...
UDPWiimote::UDPWiimote(const std::string& _port, const std::string& name, int _index)
    : port(_port), displayName(name), d(new _d), waX(0), waY(0), waZ(1), naX(0), naY(0), naZ(-1),
      nunX(0), nunY(0), pointerX(1001.0f / 2), pointerY(0), nunMask(0), wiimoteMask(0),
      index(_index), int_port(atoi(_port.c_str()))
{
    ...
    if (rt)
    {
      for (sock_t fd : d->sockfds)
      {
        if (FD_ISSET(fd, &fds))
        {
          ...
          if ((size = recvfrom(fd, (dataz)bf, size, 0, (struct sockaddr*)&their_addr,
...
int UDPWiimote::pharsePacket(u8* bf, size_t size)
{
  ...
  if (bf[2] & ACCEL_FLAG)
  {
    ...
    double ux, uy, uz;
    ux = (double)((s32)ntohl(*p));
    p++;
    uy = (double)((s32)ntohl(*p));
    p++;
    uz = (double)((s32)ntohl(*p));
    p++;
  ...
  if (bf[2] & IR_FLAG)
  {
    ...
    pointerX = ((double)((s32)ntohl(*p))) / 1048576;
    p++;
    pointerY = ((double)((s32)ntohl(*p))) / 1048576;
    p++;
  }
  ...
  if (bf[2] & NUN_FLAG)
  {
    ...
    nunMask = *((u8*)p);
    p = (u32*)(((u8*)p) + 1);
    nunX = ((double)((s32)ntohl(*p))) / 1048576;
    p++;
    nunY = ((double)((s32)ntohl(*p))) / 1048576;
    p++;
  }
...
void UDPWiimote::broadcastIPv4(const void* data, size_t size)
{
  ...
  int num;
  if ((num = sendto(d->bipv4_fd, (const dataz)data, (int)size, 0, (struct sockaddr*)&their_addr,
                    sizeof their_addr)) == -1)
  {
