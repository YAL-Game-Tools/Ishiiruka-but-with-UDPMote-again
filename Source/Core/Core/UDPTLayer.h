...
namespace UDPTLayer
{
static void GetButtons(UDPWrapper* m, u16* butt)
{
  if (!(m->inst))
    return;
  if (!(m->updButt))
...
static void GetIR(UDPWrapper* m, ControlState* x, ControlState* y, ControlState* z)
{
  ...
  if (!(m->updIR))
    return;
  // if ((*x >= -0.999) && (*x <= 0.999) && (*y >= -0.999) && (*y <= 0.999)) return; //the received
  // values are used ONLY when the normal pointer is offscreen
  float _x, _y;
  m->inst->getIR(&_x, &_y);
  // *x = _x * 2 - 1;
  // *y = -(_y * 2 - 1);
